...
/**
 * Test purement diagnostique : envoie un paquet UDP brut (sans se soucier du
 * protocole Bedrock) et regarde si N'IMPORTE QUELLE réponse arrive. Ça permet
 * de distinguer "le réseau bloque carrément l'UDP sortant" de "le protocole
 * Bedrock a un souci plus précis".
 */
function udpRoundTripTest(host, port, timeoutMs = 5000) {
  return new Promise((resolve) => {
    ...
    socket.send(Buffer.from([0x01]), port, host, (err) => {
      if (err) finish(false);
    });
...
function connectTo(host, port) {
  ...
  // Diagnostic en parallèle, n'affecte pas la vraie connexion
  udpRoundTripTest(host, port).then((ok) => {
    console.log(
      ok
        ? '[diag] Test UDP brut : une réponse a bien été reçue (le réseau sortant UDP fonctionne)'
        : '[diag] Test UDP brut : AUCUNE réponse reçue après 5s (peut indiquer un blocage réseau UDP sortant, ou un pare-feu Aternos qui ignore les paquets malformés — pas forcément un problème)'
    );
  });
  ...
  const client = createJsClient({
    host,
    port,
    // On NE saute plus le ping : il sert à détecter automatiquement la bonne
    // version du protocole Bedrock du serveur. Sans ça, le client utilise une
    // version par défaut qui peut ne pas correspondre à celle d'Aternos, ce
    // qui provoque un timeout silencieux côté serveur (paquets ignorés).
  ...
  const timeoutHandle = setTimeout(() => {
    if (!joined) {
      console.error(
        `[position] ⏱️ Timeout : toujours pas connecté à ${host}:${port} après ${CONNECT_TIMEOUT_MS / 1000}s. ` +
          'Le paquet part mais aucune réponse Bedrock ne revient — regarde la ligne [diag] juste au-dessus ' +
          'pour savoir si le souci est réseau (UDP bloqué) ou plus spécifique au protocole.'
      );
    }
  }, CONNECT_TIMEOUT_MS);
